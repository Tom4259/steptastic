...
namespace Sisus.Debugging.Settings
{
	...
	public static class DebugLogExtensionsProjectSettingsProvider
	{
		...
		private static readonly GUIContent IgnoreUnlistedChannelPrefixesLabel = new GUIContent("Ignore Unlisted Channel Prefixes", "When this is disabled Debug.Log Extensions automatically generates new channels from prefixes wrapped inside the '[' and ']' character found at the beginning of logged messages.");
		...
		private static void Apply()
		{
			...
			if(!Application.isPlaying)
			{
				...
				bool usingBuildStripping = !(bool)typeof(DebugLogExtensionsProjectSettings).GetField(nameof(DebugLogExtensionsProjectSettings.LogEnabledInBuilds), BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static).GetValue(null);
				...
				if(usingGlobalNamespace != asset.useGlobalNamespace || usingBuildStripping != shouldUseBuildStripping)
				{
					...
					if(typeof(Debug).Assembly.Location.Replace('/', '\\').StartsWith(Application.dataPath.Replace('/', '\\'), StringComparison.OrdinalIgnoreCase))
					{
		...
		private static void DrawSettingsGUI()
		{
			...
			if(GUILayout.Button("Reset All To Defaults"))
			{
				if(EditorUtility.DisplayDialog("Reset All To Defaults?", "Are you sure you want to reset all project settings to default values?\n\nThis will affect all users of the project.", "Reset All", "Cancel"))
				{
					...
					freshInstance.name = "DebugLogExtensionsProjectSettings";
