...
namespace Sisus.Debugging.Console
{
	internal abstract class ConsoleWindowPlusBase : EditorWindow, ISerializationCallbackReceiver
	{
		...
		private readonly ConcurrentQueue<Log> unprocessedMessagesQueue = new ConcurrentQueue<Log>();

		protected abstract bool WasCompilingLastFrame { get; }
		protected abstract bool InitialMessagesFetched { get; }
		protected abstract bool FirstMessageReceived { get; set; }
		...
		private void OnInspectorUpdate() => ProcessUnprocessedMessages();

		private void ProcessUnprocessedMessages()
		{
			if(unprocessedMessagesQueue.IsEmpty)
			{
				return;
			}
			...
			while(unprocessedMessagesQueue.TryDequeue(out Log log))
			{
				OnLogMessageReceived(log);
			}
		}
		...
		private void OnLogMessageReceivedThreaded(string message, string stackTrace, LogType type)
		{
			...
			unprocessedMessagesQueue.Enqueue(log);
		}

		protected abstract void OnLogMessageReceived(Log log);
